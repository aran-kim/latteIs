...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query(value = "select * from chat_message where create_date >= (select create_date from chat_message where type = \"ENTER\" and room_id = ?1 and user_id = ?2) order by create_date;", nativeQuery = true)
